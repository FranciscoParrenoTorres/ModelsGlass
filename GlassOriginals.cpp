...
{
	int  i;
	int  min = MAXIMUM_INT_ROADEF, min_area = MAXIMUM_INT_ROADEF, sum_area = 0;
	int  max = 0, max_area = 0;
	for (i = 0; i <batch_items; i++)
	{
		if (items[i].Getitem_h() < min)
			min = items[i].Getitem_h();
		if (items[i].Getitem_w() < min)
			min = items[i].Getitem_w();
		if (items[i].Getitem_h() > max)
			max = items[i].Getitem_h();
		if (items[i].Getitem_w() > max)
			max = items[i].Getitem_w();
		if (items[i].Getitem_area() > max_area)
			max_area = items[i].Getitem_area();
		if (items[i].Getitem_area() < min_area)
			min_area = items[i].Getitem_area();
		sum_area += items[i].Getitem_area();
	}
	int ndefect_cota = 0;
	for (i = 0; i < (((double)sum_area / (double(plate_w*plate_h))) + 3); i++)
	{
		ndefect_cota += plate_defect[i][1];
	}
//	statistics_ins_file.open("Statistics");
//	statistics_ins_file << file_idx.c_str() << "\t" << batch_items << "\t" << defects_nbr << "\t" << ndefect_cota << "\t" << stack_nbr << "\t" << min << "\t" << max << "\t" << min_area << "\t" << max_area << "\t" << ((double)sum_area / (double(plate_w*plate_h))) << "\t" << ((double)max_area / (double(plate_w*plate_h))) << "\n";

//	statistics_ins_file.close();


	/*
	fopen_s(&statistics_ins_file,"Statistics.txt","a+");
	fprintf(statistics_ins_file, "%s\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%.2f\t%.2f \n", file_idx.c_str(), batch_items, defects_nbr,ndefect_cota, stack_nbr, min, max, min_area, max_area, (double)sum_area / (double(plate_w*plate_h)), (double)max_area / (double(plate_w*plate_h)));
	//	// statistics_file << batch_items << "\t" << defects_nbr << "\t" << stack_nbr <<  "\t" << max_area  <<endl;
	//	// statistics_file << batch_items<<"\t"<< defects_nbr << "\t" << stack_nbr << "\t" << (sum_area/(plate_w*plate_h)) << "\t" << min << "\t" << max << "\t" << min_area << "\t" << max_area  << endl;
	fclose(statistics_ins_file);
	*/
}
int Glass::verify_main_interactive(void)
{
	...
	if (active_log) cout << endl << "\t------------------- Optimization Checker -------------------" << endl << endl;
//	// log_file << endl << "\t------------------- Optimization Checker -------------------" << endl;
	...
	batchPath = file_idx + "_batch.csv";
	solutionPath = file_idx + "_solution.csv";
	defectsPath = file_idx + "_defects.csv";
	optParamsPath = PATH_TO_INSTANCES "global_param.csv";
	/// If csv files exist and files index is equal.
	string log_name = "logs/" + file_idx + "_log.txt";
//	log_file.open(log_name.c_str());
	if (active_log) cout << endl << "\t\t----------- Start files parsing -----------" << endl;
...
/*	if (constraint_error != 0)
	...
	while (not_exit) {
		...
		case 5:
			...
			if (constraint_error != 0)
			{
				if (active_log_error) cout << "                       INVALID SOLUTION                     " << endl;
			}
			else
...
//	cout << endl << "\t------------------------------------------------------------" << endl;
	if (constraint_error != 0 )
	{
		...
		if (active_log == true)	cout << "               SOLUTION VALIDATED SUCCESSFULLY               " << endl;
	if (constraint_error != 0 || (active_log == true))
	{
		if (active_log_error) cout << "\t------------------------------------------------------------" << endl;



		if (active_log_error) cout << "\t\t------ End of constraints verification ----" << endl;
		if (active_log_error) cout << endl << "\t------------------------------------------------------------" << endl;
	}
	delete[]plate;
	plate = NULL;
	delete[]sol_items;
...
**/
void Glass::violatedConstraints(void) {
	// If constraints violation occured.
	if (constraint_error != 0) {
		if (active_log_error) cout << endl << "\t------------------------------------------------------------" << endl;
		if (active_log_error) cout << "\t                  Violated Constraints List                 " << endl;
		if (active_log_error) cout << "\t------------------------------------------------------------" << endl;
...
/*		// log_file << endl << "\t------------------------------------------------------------" << endl;
		...
		*/
		...
		if ((constraint_error & PARSE_ERROR_MASK) >> PARSE_ERROR_OFFSET) {
			if (active_log_error)	cout << "\t- Solution Tree Structure Constraint violated" << endl;
	//		// log_file << "\t- Solution Tree Structure Constraint violated" << endl;
		...
		if ((constraint_error & ITEM_PRODUCTION_ERROR_MASK) >> ITEM_PRODUCTION_ERROR_OFFSET) {
			if (active_log_error)	cout << "\t- Items production Constraint violated" << endl;
	//		// log_file << "\t- Items production Constraint violated" << endl;
		...
		if ((constraint_error & DEFECTS_SUPERPOSING_ERROR_MASK) >> DEFECTS_SUPERPOSING_ERROR_OFFSET) {
			if (active_log_error)	cout << "\t- Defects Superposing Constraint violated" << endl;
//			// log_file << "\t- Defects Superposing Constraint violated" << endl;
		...
		if ((constraint_error & DIMENSIONS_ERROR_MASK) >> DIMENSIONS_ERROR_OFFSET) {
			if (active_log_error)	cout << "\t- Nodes Dimensions Constraint violated" << endl;
//			// log_file << "\t- Nodes Dimensions Constraint violated" << endl;
		...
		if ((constraint_error & SEQUENCE_ERROR_MASK) >> SEQUENCE_ERROR_OFFSET) {
			if (active_log_error)	cout << "\t- Sequence Constraint violated" << endl;
//			// log_file << "\t- Sequence Constraint violated" << endl;
		...
		if ((constraint_error & IDENTITY_ERROR_MASK) >> IDENTITY_ERROR_OFFSET) {
			if (active_log_error)	cout << "\t- Identity Constraint violated" << endl;
...
**/
void Glass::displayPlatesAreaUsage(void) {
	double useful = 0.0;
	double waste = 0.0;
...
//	else
	...
	if (active_log) {
		...
		if (constraint_error) {
			if (active_log_error) cout << "\tERROR -- Unable to Display Plates Area Usage due to INVALID SOLUTION" << endl;
			if (active_log_error) cout << "\t--- End of Display Plates Area Usage ---" << endl;
			return;
		}
		cout << "\t---------------------- Summary --------------------------" << endl;
...
**/
void Glass::verifyItemProduction(void) {
	if (active_log)
...
//	else
	...
	for (i = 0; i < batch_items; i++) {
		...
		for (j = 0; j < useful_node; j++) {
			...
			if (items[i].Getitem_id() == sol_items[j].nodeType())
				nbr++;
		}
			...
			if ((node_x < defect_x + defect_width) && (defect_x < node_x + node_width) && (node_y < defect_y + defect_height) && (defect_y < node_y + node_height)) {
				...
				if (!(node_x <= defect_x && defect_x + defect_width <= node_x + node_width && node_y <= defect_y && defect_y + defect_height <= node_y + node_height)) {
					...
					if (active_log) {
						...
						if (active_log_error) cout << "\tERROR -- " << plate[plate_id].Getdefect(j).Getdefect_id() << " does not fit entirely in " << n << endl;
				}
			}
		}
	}
...
/*	if (active_log) {
		...
		if (error == 0 && active_log)
			cout << "\tSuperposing with defects constraint verified successfully" << endl;
		if (active_log) cout << "\t--- End of Superposing with defects constraint verification ---" << endl;
//	}
}

/**
* This function verifies dimensions constraint for first node cut G_level.
...
**/
void Glass::verifyDimensions(void) {
	if (active_log)
...
/*					if (active_log)
					...
					if (active_log_error) cout << "\tERROR -- Node " << plate[i].Getsuccessor(j).id() << ": (type: branch, width: " << width << ", height: " << height << ") has node.width greater than " << max1Cut << "mm" << endl;
					success = 0;
				}
			}
...
/*					// log_file << "\tERROR -- Node " << plate[i].Getsuccessor(j).id() << ": node.y is not equal to its plate " << plate[i].Getnode_id() << " y: " << plate[i].Getpos_y() << endl;
				...
				if (active_log_error) cout << "\tERROR -- Node " << plate[i].Getsuccessor(j).id() << ": node.y is not equal to its plate " << plate[i].Getnode_id() << " y: " << plate[i].Getpos_y() << endl;
				success = 0;
			}
...
/*					// log_file << "\tERROR -- Node " << plate[i].Getsuccessor(j).id() << ": node.height is not equal to its plate " << plate[i].Getnode_id() << " height: " << plate[i].Getheight() << endl;
				...
				if (active_log_error) cout << "\tERROR -- Node " << plate[i].Getsuccessor(j).id() << ": node.height is not equal to its plate " << plate[i].Getnode_id() << " height: " << plate[i].Getheight() << endl;
				success = 0;
			}
			// verifies if first node of plate[i] starts at coordinate node.x = 0.
			if ((j == 0) && (x != 0)) {
//				if (active_log)
				if (active_log_error) cout << "\tERROR -- Node " << plate[i].Getsuccessor(j).id() << ": (x: " << x << ", y: " << y << ", width: " << width << ", height: " << height << ") has node.x != plate.x (plate.node_id: " << plate[i].Getnode_id() << ", x: 0)" << endl;
	//			else
	...
	//			if (active_log)
		//			// log_file << "\tERROR -- Node " << plate[i].Getsuccessor(j).id() << ": (x: " << x << ", y: " << y << ", width: " << width << ", height: " << height << ") has node.x != node[" << plate[i].Getsuccessor(j - 1).id() << "].x + node[" << plate[i].Getsuccessor(j - 1).id() << "].width" << endl;
			//	else
				if (active_log_error) cout << "\tERROR -- Node " << plate[i].Getsuccessor(j).id() << ": (x: " << x << ", y: " << y << ", width: " << width << ", height: " << height << ") has node.x != node[" << plate[i].Getsuccessor(j - 1).id() << "].x + node[" << plate[i].Getsuccessor(j - 1).id() << "].width" << endl;
				success = 0;
			}
	...
	//			else
				if (active_log_error) cout << "\tERROR -- Node " << plate[i].Getsuccessor(j).id() << ": (x: " << x << ", y: " << y << ", width: " << width << ", height: " << height << ") has node.x + node.width > plate.width (plate.node_id: " << plate[i].Getnode_id() << ")" << endl;
				success = 0;
			}
	...
	//			else
				if (active_log_error) cout << "\tERROR -- Node " << plate[i].Getsuccessor(j).id() << ": is the last of plate " << plate[i].Getplate_id() << " successors and node.x + node.width < plate.width (plate.node_id: " << plate[i].Getnode_id() << ")" << endl;
				success = 0;
			}
...
/*					if (active_log)
					...
					if (active_log_error) cout << "\tERROR -- Node " << plate[i].Getsuccessor(j).id() << ": (type: waste, width: " << width << ", height: " << height << ") has node.height less than " << waste_min << "mm" << endl;
					success = 0;
				}
...
/*					if (active_log)
					...
					if (active_log_error) cout << "\tERROR -- Node " << plate[i].Getsuccessor(j).id() << ": (type: waste, width: " << width << ", height: " << height << ") has node.width less than " << waste_min << "mm" << endl;
					success = 0;
				}
			}
...
/*					if (active_log)
					...
					if (active_log_error) cout << "\tERROR -- Node " << plate[i].Getresidual().id() << ": (type: residual, width: " << plate[i].Getresidual().w() << ", height: " << plate[i].Getresidual().h() << ") has node.width less than " << waste_min << "mm" << endl;
					success = 0;
				}
			}
...
/*				// log_file << "\tERROR -- Node " << parent.Getsuccessor(j).id() << ": (type: branch, width: " << width << ", height: " << height << ") has node.cut != parent.cut + 1 (parent.node_id: " << parent.id() << ")" << endl;
				...
				if (active_log_error) cout << "\tERROR -- Node " << parent.Getsuccessor(j).id() << ": (type: branch, width: " << width << ", height: " << height << ") has node.cut != parent.cut + 1 (parent.node_id: " << parent.id() << ")" << endl;
			success = 0;
		}
	...
	/*			if (active_log)
				...
				if (active_log_error) cout << "\tERROR -- Node " << parent.Getsuccessor(j).id() << ": (type: waste, width: " << width << ", height: " << height << ") has node.height less than " << waste_min << "mm" << endl;
				success = 0;
			}
			// verifies if node.width is not inferior to waste_min optimization parameter prefixed value.
			if (width < waste_min) {
	/*			if (active_log)
				...
				if (active_log_error) cout << "\tERROR -- Node " << parent.Getsuccessor(j).id() << ": (type: waste, width: " << width << ", height: " << height << ") has node.width less than " << waste_min << "mm" << endl;
				success = 0;
			}
		}
...
/*			if (active_log)
			...
			if (active_log_error)  cout << "\tERROR -- Node " << parent.Getsuccessor(j).id() << ": (type: branch, width: " << width << ", height: " << height << ") has node.height less than " << min2Cut << "mm" << endl;
			success = 0;
		}
		// if odd cut stage.
		if (cut % 2) {
...
/*				if (active_log)
				...
				if (active_log_error) 	cout << "\tERROR -- Node " << parent.Getsuccessor(j).id() << ": node.y is not equal to its parent node " << parent.id() << " y: " << parent.y() << endl;
				success = 0;
			}
...
/*				if (active_log)
				...
				if (active_log_error) 	cout << "\tERROR -- Node " << parent.Getsuccessor(j).id() << ": node.height is not equal to its parent node " << parent.id() << " height: " << parent.h() << endl;
				success = 0;
			}
...
/*				if (active_log)
				...
				if (active_log_error) 	cout << "\tERROR -- Node " << parent.Getsuccessor(j).id() << ": (x: " << x << ", y: " << y << ", width: " << width << ", height: " << height << ") has node.x != parent.x / parent is Node " << parent.id() << "( x: " << parent.x() << ", y: " << parent.y() << " width: " << parent.w() << ", height: " << parent.h() << ")" << endl;
				success = 0;
			}
...
/*				if (active_log)
				...
				if (active_log_error) 	cout << "\tERROR -- Node " << parent.Getsuccessor(j).id() << ": (x: " << x << ", y: " << y << ", width: " << width << ", height: " << height << ") has node.x != node[" << parent.Getsuccessor(j - 1).id() << "].x + node[" << parent.Getsuccessor(j - 1).id() << "].width" << endl;
				success = 0;
			}
...
/*				if (active_log)
				...
				if (active_log_error) 	cout << "\tERROR -- Node " << parent.Getsuccessor(j).id() << ": node.x + node.width > parent.x + parent.width (parent.node_id: " << parent.id() << ")" << endl;
				success = 0;
			}
...
/*				if (active_log)
				...
				if (active_log_error) 	cout << "\tERROR -- Node " << parent.Getsuccessor(j).id() << ": is the last of node " << parent.id() << " successors and node.x + node.width < parent.x + parent.width (parent.node_id: " << parent.id() << ")" << endl;
				success = 0;
			}
		}
...
/*				if (active_log)
				...
				if (active_log_error) 	cout << "\tERROR -- Node " << parent.Getsuccessor(j).id() << ": x: " << parent.Getsuccessor(j).x() << " is not equal to its parent node " << parent.id() << " x: " << parent.x() << endl;
				success = 0;
			}
...
/*				if (active_log)
				...
				if (active_log_error) 	cout << "\tERROR -- Node " << parent.Getsuccessor(j).id() << ": node.width is not equal to its parent node " << parent.id() << " width: " << parent.w() << endl;
				success = 0;
			}
...
/*				if (active_log)
				...
				if (active_log_error) 	cout << "\tERROR -- Node " << parent.Getsuccessor(j).id() << ": (x: " << x << ", y: " << y << ", width: " << width << ", height: " << height << ") has node.y != parent.y / parent is Node " << parent.id() << "( x: " << parent.x() << ", y: " << parent.y() << " width: " << parent.w() << ", height: " << parent.h() << ")" << endl;
				success = 0;
			}
...
/*				if (active_log)
				...
				if (active_log_error) 	cout << "\tERROR -- Node " << parent.Getsuccessor(j).id() << ": (x: " << x << ", y: " << y << ", width: " << width << ", height: " << height << ") has node.y != node[" << parent.Getsuccessor(j - 1).id() << "].y + node[" << parent.Getsuccessor(j - 1).id() << "].height" << endl;
				success = 0;
			}
...
/*				if (active_log)
				...
				if (active_log_error) 	cout << "\tERROR -- Node " << parent.Getsuccessor(j).id() << ": node.y + node.height > parent.y + parent.height (parent.node_id: " << parent.id() << ")" << endl;
				success = 0;
			}
...
/*				if (active_log)
				...
				if (active_log_error) 	cout << "\tERROR -- Node " << parent.Getsuccessor(j).id() << ": is the last of node " << parent.id() << " successors and node.y + node.height < parent.y + parent.height (parent.node_id: " << parent.id() << ")" << endl;
				success = 0;
			}
		}
		if (type == -2)
...
**/
void Glass::verifyIdt_Sequence(void) {
	if (active_log)
	...
	/*		// log_file << "\tERROR -- Unable to verify Sequence & Identity constraints due to Item Production ERROR" << endl;
		...
		for (i = 0; i < useful_node; i++) {
			...
			for (j = 0; j < stack_nbr; j++) {
				curItemIdx = stacks[j].Getcur_item_idx();
				...
				if (curItemIdx < stacks[j].Getitem_nbr() && sol_items[i].nodeType() == stacks[j].Getitem(curItemIdx).Getitem_id()) {
					fnd_stck++;
					sol_width = sol_items[i].w();
					sol_height = sol_items[i].h();
					batch_width = stacks[j].Getitem(curItemIdx).Getitem_w();
					batch_height = stacks[j].Getitem(curItemIdx).Getitem_h();
...
//							// log_file << "\tERROR -- Identity constraint is not respected " << endl << "\t\tItem " << sol_items[i].nodeType() << " ( width: " << sol_width << ", height: " << sol_height << ")  parameters are not identical to its batch version (width: " << batch_width << ", height: " << batch_height << ")." << endl;
	...
	//				constraint_error |= SEQUENCE_ERROR_MASK;
		//		}
			//	else*/
				if (active_log_error)	cout << "\tERROR -- Sequence constraint is not respected " << endl << "\t\tItem " << sol_items[i].nodeType() << " is not allowed" << endl;
				break;
			}
		}
...
int Glass::parseOptimizationParams(string path) {
	...
	if (Type == 1) return EXIT_SUCCESS;
//	printf("Aqui Pasa Algo 21-1\n");
	int  i = 0;
...
int Glass::parseDefects(string path) {
	for (int i1=0;i1<PLATES_NBR_LIMIT;i1++)
	DefectsPlate[i1].clear();
	defects_nbr = 0;
	if (Type == 1)
	{
		int defects_on_used_plats = 0;
		int i = 0;
	...
	if (active_log) cout << endl << "\t--- Parsing Defects file ---" << endl << "\tDefects file path: " << path << endl;
	...
	while (filess.good()) {
		...
		if (token[DEFECTS_DEFECT_ID_COL].size()) {
			int id = atoi(token[DEFECTS_PLATE_ID_COL].c_str());
			id = id/ ManyDefects;
	...
	filess.close();
	if (active_log) cout << "\t--- Defects file parsed successfully ---" << endl << endl;
...
int Glass::parseBatch2DC(string path) {
...
int Glass::parseBatchWithoutStacksBerkey(string path) 
{


	FILE* fin2;
	fin2 = fopen(path.c_str(), "a+");
	if (fin2 == NULL)
	{
		printf("Problemas al leer el archivo del container, no se encuentra el archivo %s", path.c_str());
		exit(4);
	}
	//Leemos el número de piezas
	int kk = 0;
	fscanf(fin2, "%d %d", &batch_items, &kk);

	//	fscanf(fin2,"%d",&m_Q);
	//	m_lb=1;
	//	fscanf(fin2,"%d ",&m_Q);
		//Las coordenadas del contenedor
	fscanf(fin2, "%d %d %d", &plate_w, &plate_h, &kk);
	//	fscanf(fin2,"%d ",&m_X);
	...
	bool Rx, Ry, Rz;
	items = new GlassItem[batch_items];
	stacks = new GlassStack[batch_items];
	stack_nbr = batch_items;
	for (int register i = 0; i < batch_items; i++)
	{
	...
	//		fscanf(fin2,"%d %d %d %d %d %d %d %d" , &Id, &x, &Rx, &y, &Ry, &z, &Rz,&q);
		...
		items[i].Setitem_id(Id);
		items[i].Setitem_h(x);
		items[i].Setitem_w(y);
		items[i].Setitem_stack(Id);
		items[i].Setitem_seq(1);
		items[i].Setitem_num(1);
		stacks[i].Setstack_id(i);
		//Allocate memory for stacks's items.
		stacks[i].item = new GlassItem[1];
		stacks[i].Setitem(items[i]);
		stacks[i].Setitem_nbr(1);
		if (items[i].Getitem_min() < MinDimensionPieza_Inicial)
			MinDimensionPieza_Inicial = items[i].Getitem_min();
		if (items[i].Getitem_min() > MaxMinDimensionPieza_Inicial)
			MaxMinDimensionPieza_Inicial = items[i].Getitem_min();

		Total_area += items[i].Getitem_area();
...
int Glass::parseBatchWithoutStacks(string path) {
	if (Type == 1)
	{
		parseBatchWithoutStacksBerkey(path);
		return EXIT_SUCCESS;
	}
		int  j, i, k, p = 0, s = 0;
		int n = -1, first = 0, init = 0;
...
void Glass::parseBatchJointItems(string path)
{
	...
	while (filess.good())
	{
		...
		if (token[BATCH_ITEM_ID_COL].size())
		{
			...
			bool joint = createItemJoint(items, token, p++);
			if (joint == true)
				p--;
			else
			{
...
**/
int Glass::parseSolution(string path) {
	if (Type==1)
		return EXIT_SUCCESS;
	GlassNode *node;
	string value, temp;
	string token[SOLUTION_COL_LENGTH];
	plates_nbr = 0; // Save solution used plates number.
	for (int register i = 0; i < PLATES_NBR_LIMIT; i++)
	{
		plates_list[i][0] = 0;
		plates_list[i][1] = 0;
	}
	...
	useful_node = 0; // Solution useful nodes number (node not a branch or waste or residual).
	branch_node = 0; // Solution branch nodes number
	residual_node = 0; // Solution residual nodes number.
	waste_node = 0; // Solution waste nodes number.

	constraint_error = 0; // Success constraint, combination of solution's occured errors (refer to main.h to find errors list).


	total_waste = 0; // Sum of wasted area.
	total_useful = 0; // Sum of used area.
	...
	if (active_log) cout << endl << "\t--- Parsing Solution file ---" << endl << "\tSolution file path: " << path << endl;
//	// log_file << endl << "\t--- Parsing Solution file ---" << endl << "\tSolution file path: " << path << endl;
	...
	while (filess.good()) {
		...
		for (i = 0; i < plates_nbr; i++) {
			// If plate is already counted, increase plate's G_children number.
		...
		if (token[SOLUTION_PLATE_ID_COL].size() != 0) {
			...
			if (token[SOLUTION_PARENT_COL].size() == 0) {
				...
				if (atoi(token[SOLUTION_PLATE_ID_COL].c_str()) == p) {
					...
					if (active_log_error) cout << "\tPlate Partial_objective_function Sequence Constraint violated ";
					if (active_log_error) cout << "\t(ERROR: Plate " << atoi(token[SOLUTION_PLATE_ID_COL].c_str()) << " is used when Plate " << p << " is expected)" << endl;
					// log_file << "\tPlate Partial_objective_function Sequence Constraint violated ";
	...
	if (active_log)	cout << "\tPlates: " << plates_nbr<< "\tGlobal Iter: " << m_id <<endl;
	
//	// log_file << "\tPlates: " << plates_nbr;
	if (plates_nbr > plate_nbr_limit) {
		if (active_log_error) cout << " (ERROR: Exceed plates number limit which's " << plate_nbr_limit << ")" << endl;
...
//	// log_file << "\tbranches: " << branch_node << endl << "\tItems: " << useful_node << endl << "\tResiduals: " << residual_node;
	if (residual_node > 1) {
		if (active_log_error) cout << " (ERROR: Solution should have one and only one residual item located on the rightmost node of the last plate) Iter"<< m_id ;
//		// log_file << " (ERROR: Solution should have one and only one residual item located on the rightmost node of the last plate)";
		constraint_error |= PARSE_ERROR_MASK;
	}
	if (active_log) cout << endl << "\tWaste: " << waste_node;
//	// log_file << endl << "\tWaste: " << waste_node;
	if (active_log) cout << endl << "\tUsed Cut Stages: " << max_cut_stage;
...
//	cout << endl; // << " waste: " << node_nbr - (useful_node + branch_node) << endl;
	...
	if (active_log_error)	cout << "\t--- ERROR -- Solution file Contains an INVALID SOLUTION ---" << endl;
	//	// log_file << "\t--- ERROR -- Solution file Contains an INVALID SOLUTION ---" << endl;

	}
	else {
		if (active_log) cout << "\tTree structure built" << endl;
	//	// log_file << "\tTree structure built" << endl;
	}

	if (active_log) cout << "\t--- Solution file parsed successfully ---" << endl;
...
* to build the first G_level of the tree structure.
			...
			if (node[index + j].Getparent() == plate[i].Getnode_id()) {
				...
				case -3: // residual.
					if (node[index + j].plateId() == (plates_nbr - 1)) {
						if (j == c_nbr - 1) {
						if (active_log)	cout << "\tNode " << node[index + j].id() << ": is a residual" << endl;
						//	// log_file << "\tNode " << node[index + j].id() << ": is a residual" << endl;
							plate[node[index + j].plateId()].Setresidual(node[index + j]);
						}
						else {
							if (active_log_error) cout << "\tERROR -- Node " << node[index + j].id() << ": is a residual (ERROR: Residual item should be the rightmost node of the last plate)" << endl;
						...
						if (active_log_error) cout << "\tERROR -- Node " << node[index + j].id() << ": is a residual (ERROR: Residual item can be located only on the last plate)" << endl;
				...
				case -1: // waste.
					if ((node[index + j].plateId() == (plates_nbr - 1)) && (j == c_nbr - 1)) {
						if (active_log)	cout << "\tNode " << node[index + j].id() << ": is a WASTE that should be considered as a RESIDUAL" << endl;
...
**/
void Glass::checkSuccessors(GlassNode *node, GlassNode *sol_items, int plate_id, GlassNode &parent, int c_end, int index) {
	int i, nbr = 0;
	int waste = 0.0, useful = 0.0;
	...
	for (i = index; i < c_end; i++) {
		if (node[i].Getparent() == parent.id()) {
			...
			if (parent.Setsuccessor(node[i]) == 1) {
				if (active_log_error) cout << "\tERROR -- Node: " << parent.id() << " is " << parent.Getcut() << "-cut and has more than 2 successors" << endl;
				// log_file << "\tERROR -- Node: " << parent.id() << " is " << parent.Getcut() << "-cut and has more than 2 successors" << endl;
				constraint_error |= PARSE_ERROR_MASK;
			}
			switch (node[i].nodeType()) {
			case -3: // residual.
				if (active_log_error) cout << "\tERROR -- Node " << node[i].id() << ": is a residual (ERROR: residual must be 1-cut node)" << endl;
...
**/
int Glass::createPlate(string *token, int p) {
...
**/
int Glass::createNode(GlassNode *node, string *token, int n) {
	...
	if (isNegative(node[n].id()) || isNegative(node[n].plateId()) || isNegative(node[n].Getcut()) ||
		...
		if (active_log_error) if (active_log_error) cout << "\tERROR: One attribute of a node is negative or null, please check Solution file" << endl;
		return EXIT_FAILURE;
	}
	return EXIT_SUCCESS;
}
...
* node: input / node array.
**/
int Glass::createItem(GlassItem *items, string *token, int p)
{
	...
	if (Duplicated==1)
	{
		if ((items[p].Getitem_h() * 2)<plate_h)
		items[p].Setitem_h(items[p].Getitem_h() * 2);
		if ((items[p].Getitem_w() * 2) < plate_h)
		items[p].Setitem_w(items[p].Getitem_w() * 2);
	}
	if (Duplicated == 2)
	{
		if ((items[p].Getitem_h() * 3) < plate_h)
			items[p].Setitem_h(items[p].Getitem_h() * 3);
		if ((items[p].Getitem_w() * 3) < plate_h)
			items[p].Setitem_w(items[p].Getitem_w() * 3);
	}
	items[p].Setitem_stack(atoi(token[BATCH_STACK_COL].c_str()));
	items[p].Setitem_seq(atoi(token[BATCH_SEQ_COL].c_str()));
	items[p].Setitem_num(1);
	if (Type == 2)
	{
		items[p].Setitem_value(atoi(token[5].c_str()));
	}
	else items[p].Setitem_value(1);

	//Check for negative value
	if (isNegative(items[p].Getitem_id()) || isNegative(items[p].Getitem_stack()) || isNegative(items[p].Getitem_seq()) ||
		isNegativeOrNull(items[p].Getitem_h()) || isNegativeOrNull(items[p].Getitem_w())) {
		if (active_log_error) cout << "\tERROR: One attribute of an item is negative or null, please check Batch file" << endl;
		return EXIT_FAILURE;
	}
	return EXIT_SUCCESS;
}
bool Glass::createItemJoint(GlassItem *items, string *token, int p)
{
	if (p >= 1)
	{
		if ((items[p - 1].Getitem_stack() == atoi(token[BATCH_STACK_COL].c_str())))
		{
			if ((items[p - 1].Getitem_h() == atoi(token[BATCH_LENGTH_ITEM_COL].c_str())))
			{
				if ((items[p - 1].Getitem_w() == atoi(token[BATCH_WIDTH_ITEM_COL].c_str())))
				{
					items[p - 1].Add_num();
					return true;
				}
